...
- **Contradiction judge is advisory (LAB-3283 Phase 1)** — an LLM judge (`ContradictionJudge`, Haiku via anthropic-lb by default) classifies every `CONTRADICTS` pair as `contradiction` / `supersession` / `coexist` / `unrelated`, off the store path (`spawn_local` after store, plus `POST /backfill/contradictions`). Verdicts are written onto the edge via the bridge (`POST /contradictions/verdict`, MATCH-only) and one `tracing` event on target `alaya::judge` records `would_supersede` per verdict — the shadow log Phase 2 (LAB-3285) is promoted on. Failures are classified: transient (429/5xx/timeout/misconfig) write nothing and are retried; deterministic (schema/parse/empty/400) persist `verdict = unjudged` + the error as reason so the backfill's NULL selection skips the pair (`rejudge: true` re-selects edges judged by another model). Never a vector write, never a panic. `memory_contradictions` filters entirely in Cypher (`ContradictionQuery`: verdicts, `exclude_resolved` via incoming `SUPERSEDES` edges, `SKIP`/`LIMIT`) and pages with `offset`/`next_offset` — app-side filtering over a LIMIT-only read starved the queue (review, 2026-09-10).
